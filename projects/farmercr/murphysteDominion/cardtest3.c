...
* Description: A card test for Council Room card function.
* Websites consulted:
...
int main()
{
	...
	// test player buy count increased by 1
	assertResult((startGame.numBuys + 1), testGame.numBuys, "Player buy count");

	// test that player gained 4 cards
	assertResult((startGame.handCount[thisPlayer] + 4 - 1), testGame.handCount[thisPlayer], "Player hand count");

	// test that the player deck decreased by 4 cards
	assertResult((startGame.deckCount[thisPlayer] - 4), testGame.deckCount[thisPlayer], "Player deck count");
